...
本项目不设固定发布频率。只有选题存在可复现缺口、三轮上下文隔离 AI 审计通过且公开 CI 全绿时才发布，不为日历制造空版本。

路线采用**累计式演进**：表中每一行只写本版的新增主题，不表示该版只拥有这些能力。自 v0.1 起，后续 `src` 与默认 CLI 必须继续保留终端启动、多轮内存上下文、Bash 和逐次审批；冻结 lesson 则在这条基线上展示当时新增的最小抽象。完整继承关系见 [`docs/capability-matrix.md`](docs/capability-matrix.md)。
...
| `v0.6.0` | TUI 状态与文本帧 | 输入区、消息流、工具卡片、状态栏的确定性投影 |
