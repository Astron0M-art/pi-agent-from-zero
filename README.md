...
当前版本：[`v0.6.0` TUI 状态与文本帧渲染基础](lessons/06-tui-basics/README.md)。它把 Agent 事件投影为输入区、消息时间线、工具卡片、固定状态栏和有限视口，并保持显示状态与模型上下文分离。当前演示只输出一次确定性文本帧，**不是**支持 raw mode、键盘事件循环和差分重绘的完整交互式 TUI。
