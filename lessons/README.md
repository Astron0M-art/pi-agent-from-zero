...
每个版本会在这里保留一份独立可运行的阶段快照。课程按照目录编号顺序学习：每一版继承上一版的终端、多轮、Bash 与审批基线，再增加表中的核心问题；它们不是六个互不相干的 Demo。最新工程实现位于 `src/pi_agent_from_zero/`，逐项证据见[能力矩阵](../docs/capability-matrix.md)。
...
| `v0.6.0` | [TUI 状态与文本帧](06-tui-basics/README.md) | 如何把 Agent 事件投影成输入区、消息时间线、工具卡片与状态栏，而不污染模型上下文？ |
